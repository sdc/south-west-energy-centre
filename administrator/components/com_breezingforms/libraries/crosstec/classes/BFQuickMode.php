			...
			if($mdata['type'] == 'page'){
				...
				if(!$mdata['hideLabel']){
					...
					if(trim($mdata['hint']) != ''){
						...
						$tipScript   = '<script type="text/javascript"><!--'."\n".'JQuery(document).ready(function() {JQuery("#bfLabel'.$mdata['dbId'].'").qtip({ position: { adjust: { screen: true } }, content: "<b>'.addslashes(trim($mdata['label'])).'</b><br/>'.str_replace( array("\n","\r"), array("\\n",""), addslashes($content) ).'"'.$style.' });});'."\n".'//--></script>';
                                            }
                                        }
					...
					case 'bfFile':
						...
						if(isset( $mdata['flashUploader'] ) && $mdata['flashUploader']){
							...
							var bfFlashFileQueue".$mdata['dbId']." = {};
									...
									'script'         : '".JURI::root(true).(JFactory::getConfig()->getValue('config.sef') && !JFactory::getConfig()->getValue('config.sef_rewrite') ? '/index.php/' : '/').(JRequest::getCmd('lang','') && JFactory::getConfig()->getValue('config.sef') ? JRequest::getCmd('lang','') . ( JFactory::getConfig()->getValue('config.sef_rewrite') ? '/index.php' : '/' ) : 'index.php')."',
