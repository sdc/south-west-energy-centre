...
This is an altered version of the core Joomla content search plugin so the search results get filtered by access as set in component Access Manager.
 ...
 * @since		1.6
 */
class plgSearchContentaccessmanager extends JPlugin{
	/**
	...
	function onContentSearchAreas(){
		//static $areas = array(
			...
			//);
		static $areas = array('contentaccessmanager' => 'JGLOBAL_ARTICLES');
		return $areas;
	}
	...
	function onContentSearch($text, $phrase='', $ordering='', $areas=null){
		...
		if ($sContent && $limit > 0)
		{
			...
			$where_articles_categories = ' AND a.access IN ('.$groups.') AND c.access IN ('.$groups.') ';
			$access_script_file = JPATH_ROOT.DS.'components'.DS.'com_accessmanager'.DS.'checkaccess.php';
			if(file_exists($access_script_file)){
			
				require_once($access_script_file);
				$access_script = new accessmanagerAccessChecker();
				$am_config = $access_script->get_config();
				if($am_config['am_enabled'] && ($am_config['article_active'] || $am_config['category_active'])){				
					$where_articles_categories = ' AND '.$access_script->where_articles_categories('a', 'c', 'a.access IN ('.$groups.')', 'c.access IN ('.$groups.')').' ';	
				}				
			}					
			$query->where('('. $where .')' . 'AND a.state=1 AND c.published = 1 '
						.$where_articles_categories											
		...
		if ($sArchived && $limit > 0)
		{
			...
			$query->where('('. $where .') AND a.state = 2 AND c.published = 1 '
				.$where_articles_categories	
