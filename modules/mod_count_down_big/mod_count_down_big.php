...
$date_array = mod_count_down_bigHelper::dateArray($params);

// Get time to event and countdown status
list($days_left, $hours_left, $minutes_left, $seconds_left, $up) = mod_count_down_bigHelper::countdown($date_array[0]);
list($eventdate, $oclock) = mod_count_down_bigHelper::dateformat($params, $date_array[1], $date_array[2], $date_array[3], $date_array[4]);

$days_switch = mod_count_down_bigHelper::daysname($days_left, $up);
...
if($dynamic == 1)
{
    mod_count_down_bigHelper::countdowndyn($eventup, $module_id);
}
