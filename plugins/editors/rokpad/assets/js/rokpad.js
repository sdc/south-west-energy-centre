...
return this;};d.cancelFrame=function(h){g.cancel(h);return this;};d.color=a;e.exports=d;},"1":function(k,e,y,s){var m={maroon:"#800000",red:"#ff0000",orange:"#ffA500",yellow:"#ffff00",olive:"#808000",purple:"#800080",fuchsia:"#ff00ff",white:"#ffffff",lime:"#00ff00",green:"#008000",navy:"#000080",blue:"#0000ff",aqua:"#00ffff",teal:"#008080",black:"#000000",silver:"#c0c0c0",gray:"#808080",transparent:"#0000"};
...
}C/=360;G/=100;B/=100;F/=1;if(C>1||C<0||G>1||G<0||B>1||B<0||F>1||F<0){return null;}if(G===0){c=E=D=B;}else{var z=B<0.5?B*(1+G):B+G-B*G;var A=2*B-z;c=n(A,z,C+1/3);
D=n(A,z,C);E=n(A,z,C-1/3);}return[c*255,D*255,E*255,F];};var r=[];for(var v in m){r.push(v);}var o="(?:#([a-f0-9]{3,8}))",f="\\s*([.\\d%]+)\\s*",b="(?:,\\s*([.\\d]+)\\s*)?",u="\\("+[f,f,f]+b+"\\)",g="(?:rgb)a?",q="(?:hsl)a?",p="("+r.join("|")+")";
...
}if(A){return c;}if(c[3]===1){c.splice(3,1);}return"rgb"+(c.length===4?"a":"")+"("+c+")";};t.x=RegExp([p,o,g+u,q+u].join("|"),"gi");e.exports=t;},"2":function(c,b,f,a){var d=a.requestAnimationFrame||a.webkitRequestAnimationFrame||a.mozRequestAnimationFrame||a.oRequestAnimationFrame||a.msRequestAnimationFrame||function(m){return setTimeout(m,1000/60);
};var j=[],h=false;var g=function(n){if(n==null){n=+(new Date);}h=false;for(var m=j.length;m--;){var o=j.shift();if(o){o(n);}}};var k=function(m){for(var n=j.length;
n--;){if(j[n]===m){j.splice(n,1);break;}}};var e=function(m){j.push(m);if(!h){h=true;d(g);}return function(){k(m);};};f.request=e;f.cancel=k;},"3":function(k,g,ag,c){var O=k("1"),b=k("2");
var ae=b.cancel,X=b.request;var E=k("4"),o=k("5"),z=k("7");var al=z.camelize,ab=z.clean,P=z.capitalize;var N=o.map,af=o.forEach,F=o.indexOf;var Y=k("9");
var H=k("a");var R={};var m=function(e){return R[e]||(R[e]=z.hyphenate(e));};var ai=function(e){return Math.round(e*1000)/1000;};var w=c.getComputedStyle?function(au){var e=getComputedStyle(au);
return function(av){return e?e.getPropertyValue(m(av)):"";};}:function(au){var e=au.currentStyle;return function(av){return e?e[al(av)]:"";};};var s=document.createElement("div");
var L="border:none;margin:none;padding:none;visibility:hidden;position:absolute;height:0;";var aj=function(au,e){var aw=au.parentNode,av=1;if(aw){s.style.cssText=L+("width:100"+e+";");
aw.appendChild(s);av=s.offsetWidth/100;aw.removeChild(s);}return av;};var I=function(e){var au=e.length;if(au===1){e.push(e[0],e[0],e[0]);}else{if(au===2){e.push(e[0],e[1]);
}else{if(au===3){e.push(e[1]);}}}return e;};var q="([-.\\d]+)(%|cm|mm|in|px|pt|pc|em|ex|ch|rem|vw|vh|vm)",t=q+"?",am="none|hidden|dotted|dashed|solid|double|groove|ridge|inset|outset|inherit";
var d=RegExp(q,"g"),aq=RegExp(t),J=RegExp(t,"g"),C=RegExp(am);var V=function(e){return e==null?"":e+"";};var j=function(au,e){if(au==null||au===""){return e?"1":"";
}return isFinite(au=+au)?au<0?"0":au+"":"1";};try{s.style.color="rgba(0,0,0,0.5)";}catch(ao){}var ah=/^rgba/.test(s.style.color);var v=function(au,e){var ax="rgba(0,0,0,1)",aw;
if(!au||!(aw=O(au,true))){return e?ax:"";}if(e){return"rgba("+aw+")";}var av=aw[3];if(av===0){return"transparent";}return !ah||av===1?"rgb("+aw.slice(0,3)+")":"rgba("+aw+")";
};var aa=function(av,e){if(av==null||av===""){return e?"0px":"";}var au=z.match(av,aq);return au?au[1]+(au[2]||"px"):av;};var h=function(av,e){if(av==null||av===""){return e?"none":"";
}var au=av.match(C);return au?av:e?"none":"";};var U=function(aw,au){var ax="0px none rgba(0,0,0,1)";if(aw==null||aw===""){return au?ax:"";}if(aw===0||aw==="none"){return au?ax:aw+"";
}var ay;aw=aw.replace(O.x,function(az){ay=az;return"";});var av=aw.match(C),e=aw.match(J);return ab([aa(e?e[0]:"",au),h(av?av[0]:"",au),v(ay,au)].join(" "));
};var u=function(au,e){if(au==null||au===""){return e?"0px 0px 0px 0px":"";}return ab(I(N(ab(au).split(" "),function(av){return aa(av,e);})).join(" "));
...
}var au=[],aw=ab(aw).replace(O.x,function(az){au.push(az);return"";});return N(aw.split(","),function(aE,aB){var aD=v(au[aB],av),az=/inset/.test(aE),aC=aE.match(J)||["0px"];
aC=N(aC,function(aF){return aa(aF,av);});while(aC.length<e){aC.push("0px");}var aA=az?["inset",aD]:[aD];return aA.concat(aC).join(" ");}).join(", ");};
var G=function(au,e){if(au==null||au===""){return"";}return au.replace(O.x,function(av){return v(av,e);}).replace(d,function(av){return aa(av,e);});};var p={},r={},y={},S={};
var M=function(e){return p[e]||(p[e]=function(){var au=S[e]||e,av=y[e]||G;return function(){return av(w(this)(au),true);};}());};var Z=function(e){return r[e]||(r[e]=function(){var au=S[e]||e,av=y[e]||G;
return function(aw){this.style[au]=av(aw,false);};}());};var A=["Top","Right","Bottom","Left"],at=["TopLeft","TopRight","BottomRight","BottomLeft"];af(A,function(au){var e="border"+au;
af(["margin"+au,"padding"+au,e+"Width",au.toLowerCase()],function(av){y[av]=aa;});y[e+"Color"]=v;y[e+"Style"]=h;y[e]=U;p[e]=function(){return[M(e+"Width").call(this),M(e+"Style").call(this),M(e+"Color").call(this)].join(" ");
};});af(at,function(e){y["border"+e+"Radius"]=aa;});y.color=y.backgroundColor=v;y.width=y.height=y.fontSize=y.backgroundSize=aa;af(["margin","padding"],function(e){y[e]=u;
p[e]=function(){return N(A,function(au){return M(e+au).call(this);},this).join(" ");};});y.borderWidth=u;y.borderStyle=function(au,e){if(au==null||au===""){return e?I(["none"]).join(" "):"";
}au=ab(au).split(" ");return ab(I(N(au,function(av){h(av,e);})).join(" "));};y.borderColor=function(au,e){if(!au||!(au=z.match(au,O.x))){return e?I(["rgba(0,0,0,1)"]).join(" "):"";
}return ab(I(N(au,function(av){return v(av,e);})).join(" "));};af(["Width","Style","Color"],function(e){p["border"+e]=function(){return N(A,function(au){return M("border"+au+e).call(this);
},this).join(" ");};});y.borderRadius=u;p.borderRadius=function(){return N(at,function(e){return M("border"+e+"Radius").call(this);},this).join(" ");};
y.border=U;p.border=function(){var av;for(var e=0;e<A.length;e++){var au=M("border"+A[e]).call(this);if(av&&au!==av){return null;}av=au;}return av;};y.zIndex=V;
y.opacity=j;var ac=s.style.MsFilter!=null&&"MsFilter"||s.style.filter!=null&&"filter";if(ac&&s.style.opacity==null){var x=/alpha\(opacity=([\d.]+)\)/i;
r.opacity=function(au){au=(au=j(au))==="1"?"":"alpha(opacity="+Math.round(au*100)+")";var e=w(this)(ac);return this.style[ac]=x.test(e)?e.replace(x,au):e+" "+au;
};p.opacity=function(){var e=w(this)(ac).match(x);return(!e?1:e[1]/100)+"";};}var Q=y.boxShadow=function(au,e){return a(au,e,4);};var K=y.textShadow=function(au,e){return a(au,e,3);
};af(["Webkit","Moz","ms","O",null],function(e){af(["transition","transform","transformOrigin","transformStyle","perspective","perspectiveOrigin","backfaceVisibility"],function(au){var av=e?e+P(au):au;
if(e==="ms"){R[av]="-ms-"+m(au);}if(s.style[av]!=null){S[au]=av;}});});var n=S.transition,D=S.transform;if(n==="OTransition"){n=null;}var ad,W;if(!n&&D){(function(){var ax=k("c");
...
return["translate("+N(aC[0],function(aD){return ai(aD)+"px";})+")","rotate("+ai(aC[1]*180/Math.PI)+"deg)","skewX("+ai(aC[2]*180/Math.PI)+"deg)","scale("+N(aC[3],ai)+")"].join(" ");
...
}var aB=aC.split(",");if(!aB[1]){aB[1]="0px";}return N(aB,ab)+"";},translateX:av,translateY:av,scale:function(aC){if(!aC){aC="1,1";}var aB=aC.split(",");
if(!aB[1]){aB[1]=aB[0];}return N(aB,ab)+"";},scaleX:e,scaleY:e,rotate:aA,skewX:aA,skewY:aA};W=E({constructor:function(aC){var aD=this.names=[];var aB=this.values=[];
aC.replace(/(\w+)\(([-.\d\s\w,]+)\)/g,function(aF,aE,aG){aD.push(aE);aB.push(aG);});},identity:function(){var aB=[];af(this.names,function(aC){var aD=ay[aC];
...
s.style.cssText=L+m(D)+":"+aC+";";document.body.appendChild(s);var aB=w(s)(D);if(!aB||aB==="none"){aB="matrix(1, 0, 0, 1, 0, 0)";}document.body.removeChild(s);
return az(aB);}});W.prototype.toString=function(aC){var aB=this.values,aD=[];af(this.names,function(aE,aF){var aG=ay[aE];if(!aG){return;}var aH=aG(aB[aF]);
if(!aC||aH!==aG()){aD.push(aE+"("+aH+")");}});return aD.length?aD.join(" "):"none";};W.union=function(aE,aD){if(aE===aD){return;}var aB,aC;if(aE==="none"){aC=new W(aD);
aD=aC.toString();aE=aC.identity();aB=new W(aE);}else{if(aD==="none"){aB=new W(aE);aE=aB.toString();aD=aB.identity();aC=new W(aD);}else{aB=new W(aE);aE=aB.toString();
aC=new W(aD);aD=aC.toString();}}if(aE===aD){return;}if(!aB.sameType(aC)){aE=aB.decompose();aD=aC.decompose();}if(aE===aD){return;}return[aE,aD];};ad=y.transform=function(aB){if(!aB||aB==="none"){return"none";
}return(new W(aw.test(aB)?az(aB):aB)).toString(true);};p.transform=function(){var aB=this.style;return aB[D]||(aB[D]=ad(w(this)(D)));};})();}var ap=function(av,aw,az){var ay=y[aw]||G,ax=M(aw).call(av),az=ay(az,true);
if(ax===az){return;}if(ay===aa||ay===U||ay===u){var e=az.match(d),au=0;if(e){ax=ax.replace(d,function(aD,aG,aE){var aA=e[au++],aF=aA.match(aq),aC=aF[2];
if(aE!==aC){var aB=aE==="px"?aG:aj(av,aE)*aG;return ai(aB/aj(av,aC))+aC;}return aD;});}if(au>0){Z(aw).call(av,ax);}}else{if(ay===ad){return W.union(ax,az);
}}return ax!==az?[ax,az]:null;};var T=E({inherits:H,constructor:function T(av,aw){var au=M(aw),e=Z(aw);this.get=function(){return au.call(av);};this.set=function(ax){return e.call(av,ax);
};T.parent.constructor.call(this,this.set);this.node=av;this.property=aw;}});var ak;ak=E({inherits:T,constructor:function ak(){return ak.parent.constructor.apply(this,arguments);
...
}ak.parent.start.apply(this,e);if(!this.cancelStep){return this;}var au=y[this.property]||G;if((au===Q||au===K||au===G)&&this.templateFrom!==this.templateTo){this.cancelStep();
...
var ar=function(aw,au,e,ax){var av=F(au,aw);if(av!==-1){au.splice(av,1);e.splice(av,1);ax.splice(av,1);}};var an=E({inherits:T,constructor:function an(au,av){an.parent.constructor.call(this,au,av);
this.hproperty=m(S[av]||av);var e=this;this.bSetTransitionCSS=function(aw){e.setTransitionCSS(aw);};this.bSetStyleCSS=function(aw){e.setStyleCSS(aw);};
...
if(!e){this.cancel(au);return this;}this.to=e[1];this.cancelSetTransitionCSS=X(this.bSetTransitionCSS);return this;},setTransitionCSS:function(e){delete this.cancelSetTransitionCSS;
this.resetCSS(true);this.cancelSetStyleCSS=X(this.bSetStyleCSS);},setStyleCSS:function(au){delete this.cancelSetStyleCSS;var e=this.duration;this.cancelComplete=setTimeout(this.bComplete,e);
...
this.set(this.get());}}}}}return this;},resetCSS:function(au){var ay=w(this.node),ax=ay(n+"Property").replace(/\s+/g,"").split(","),aw=ay(n+"Duration").replace(/\s+/g,"").split(","),e=ay(n+"TimingFunction").replace(/\s+/g,"").match(/cubic-bezier\([\d-.,]+\)/g);
...
av[n+"Property"]=ax;av[n+"Duration"]=aw;av[n+"TimingFunction"]=e;},parseEquation:function(e){if(typeof e==="string"){return an.parent.parseEquation.call(this,e,true);
}}});var f=n?an:ak;var B=function(e,au){return typeof e==="function"?H(e):Y(e,au);};Y.implement({animate:function(aw,au,e){var aC=aw,aD=au;if(typeof aw==="string"){aC={};
...
aD.callback=function(aE){if(++ax===av){aB(aE);}};for(var aA in aC){var az=aC[aA],aA=al(aA);this.handle(function(aF){av++;var aE=this._animations||(this._animations={});
var aG=aE[aA]||(aE[aA]=new f(aF,aA));aG.setOptions(aD).start(az);});}return this;},style:function(e,ay){var au=e;if(typeof e==="string"){au={};au[e]=ay;
}for(var aw in au){var av=au[aw],ax=Z(aw=al(aw));this.handle(function(aA){var az=this._animations,aB;if(az&&(aB=az[aw])){aB.stop(true);}ax.call(aA,av);
});}return this;},compute:function(av){av=al(av);var e=this[0];if(av==="transform"&&ad){return w(e)(D);}var au=M(av).call(e);return au!=null?au.replace(d,function(aw,ay,ax){return ax==="px"?aw:aj(e,ax)*ay+"px";
}):"";}});B.parse=function(av,au,e){return(y[al(av)]||G)(au,e);};g.exports=B;},"4":function(g,d,h,c){var n=function(o,p){return Object.hasOwnProperty.call(o,p);
};var m=function(o,r,q){for(var p in o){if(r.call(q,o[p],p,o)===false){break;}}return o;};if(!{valueOf:0}.propertyIsEnumerable("valueOf")){var f="constructor,toString,valueOf,hasOwnProperty,isPrototypeOf,propertyIsEnumerable,toLocaleString".split(","),k=Object.prototype;
m=function(o,t,r){var q=f.length,p,s;for(p in o){if(t.call(r,o[p],p,o)===false){return o;}}while(q--){p=f[q];s=o[p];if(s!==k[p]&&t.call(r,s,p,o)===false){break;
}}return o;};}var j=Object.create||function(o){var p=function(){};p.prototype=o;return new p;};var e=function(o,p){this.prototype[o]=p;};var b=function(o){m(o,function(q,p){if(p!=="constructor"&&p!=="inherits"&&p!=="mutator"){this.mutator(p,q);
}},this);return this;};var a=function(p){var q=p.inherits,s;if(q){s=q.prototype;}var o=n(p,"constructor")?p.constructor:q?function(){return s.constructor.apply(this,arguments);
}:function(){};if(q){var r=o.prototype=j(s);o.parent=s;r.constructor=o;}o.mutator=p.mutator||q&&q.mutator||e;o.implement=b;return o.implement(p);};a.each=m;
a.has=n;a.create=j;d.exports=a;},"5":function(e,d,g,c){var k=e("6");var j=Array.prototype;var m=k({filter:j.filter||function(s,r){var q=[];for(var p=0,o=this.length>>>0;
p<o;p++){if(p in this){var t=this[p];if(s.call(r,t,p,this)){q.push(t);}}}return q;},indexOf:j.indexOf||function(q,r){for(var o=this.length>>>0,p=r<0?Math.max(0,o+r):r||0;
p<o;p++){if(p in this&&this[p]===q){return p;}}return -1;},map:j.map||function(s,r){var t=this.length>>>0,q=Array(t);for(var p=0,o=t;p<o;p++){if(p in this){q[p]=s.call(r,this[p],p,this);
}}return q;},forEach:j.forEach||function(r,q){for(var p=0,o=this.length>>>0;p<o;p++){if(p in this){r.call(q,this[p],p,this);}}},every:j.every||function(r,q){for(var p=0,o=this.length>>>0;
p<o;p++){if(p in this&&!r.call(q,this[p],p,this)){return false;}}return true;},some:j.some||function(r,q){for(var p=0,o=this.length>>>0;p<o;p++){if(p in this&&r.call(q,this[p],p,this)){return true;
}}return false;}});m.isArray=Array.isArray||function(o){return Object.prototype.toString.call(o)==="[object Array]";};var f={};var n="pop,push,reverse,shift,sort,splice,unshift,concat,join,slice,lastIndexOf,reduce,reduceRight".split(",");
for(var h=0,b,a;b=n[h++];){if(a=j[b]){f[b]=a;}}m.implement(f);d.exports=m;},"6":function(b,e,a,f){var d=b("4"),g=Array.prototype.slice;var c=d({mutator:function(h,j){this[h]=function(k){var m=arguments.length>1?g.call(arguments,1):[];
return j.apply(k,m);};this.prototype[h]=j;},constructor:{prototype:{}}});e.exports=function(h){var j=h.inherits||(h.inherits=c);h.constructor=d.create(j);
...
}});}e.exports=b;},"8":function(e,d,g,c){var m=e("6");var j=String.prototype;var k=m({trim:j.trim||function(){return(this+"").replace(/^\s+|\s+$/g,"");
}});var f={};var n="charAt,charCodeAt,concat,indexOf,lastIndexOf,match,quote,replace,search,slice,split,substr,substring,toLowerCase,toUpperCase".split(",");
for(var h=0,b,a;b=n[h++];){if(a=j[b]){f[b]=a;}}k.implement(f);d.exports=k;},"9":function(g,f,h,e){var c=g("4");var n=0;var a=function(q){return q===e?"global":q.uniqueNumber||(q.uniqueNumber="n:"+(n++).toString(36));
};var b={};var p,k;var j=c({constructor:function d(s,r){if(s==null){return null;}if(s instanceof o){return s;}var A=new o;if(s.nodeType||s===e){A[A.length++]=s;
}else{if(typeof s==="string"){if(p){p(s,r,A);}}else{if(s.length){var z={};for(var x=0,u=s.length;x<u;x++){var q=j(s[x],r);if(q&&q.length){for(var w=0,v=q.length;
w<v;w++){var t=q[w],y=a(t);if(!z[y]){A[A.length++]=t;z[y]=true;}}}}if(k&&A.length>1){k(A);}}}}if(!A.length){return null;}if(A.length===1){var y=a(A[0]);
return b[y]||(b[y]=A);}return A;}});var o=c({inherits:j,constructor:function o(){this.length=0;},handle:function m(v){var q=[],u=this.length;if(u===1){var s=v.call(this,this[0],0,q);
if(s!=null&&s!==false&&s!==true){q.push(s);}}else{for(var r=0;r<u;r++){var t=this[r],s=v.call(j(t),t,r,q);if(s===false||s===true){break;}if(s!=null){q.push(s);
}}}return q;}});j.use=function(q){j.implement(c.create(q.prototype));if(q.search){p=q.search;}if(q.sort){k=q.sort;}return this;};f.exports=j;},a:function(h,b,t,p){var c=h("4"),a=h("2").request,m=h("b");
var s=h("5").map;var g="([\\d.]+)(s|ms)?",k="cubic-bezier\\(([-.\\d]+),([-.\\d]+),([-.\\d]+),([-.\\d]+)\\)";var r=RegExp(g),e=RegExp(k),o=RegExp(k,"g");
...
}this.render(this.isArray?y.split(","):this.isNumber?+y:y);if(z!==1){this.cancelStep=a(this.bStep);}else{delete this.cancelStep;this.callback(u);}}});var d=function(v){var u=new f(v);
return{start:function(z,y,w){var x=typeof w;u.setOptions(x==="function"?{callback:w}:x==="string"||x==="number"?{duration:w}:w).start(z,y);return this;
},stop:function(){u.stop();return this;}};};d.prototype=f.prototype;b.exports=d;},b:function(b,c,a,d){c.exports=function(h,k,f,j,n){var g=function(p){var o=1-p;
...
if(s<w){return e(w);}if(s>v){return e(v);}while(w<v){p=g(s);if(Math.abs(p-o)<n){return e(s);}if(o>p){w=s;}else{v=s;}s=(v-w)*0.5+w;}return e(s);};};},c:function(e,d,f,b){var c=function(k){return Math.sqrt(k[0]*k[0]+k[1]*k[1]);
...
e.store("rokpad:states:fullscreen",true);}f.retrieve("twipsy").hide();this.editors[c].getEditor().focus();},insertShortCode:function(c,d,h,n){var g=n.get("data-rokpad-shortcode").replace(/'/g,'"'),m=this.editors[d].getEditor(),o=m.getSession(),p,j,e=[],b=[];
...
}b.getSession().setMode(c);return this;},setUseSoftTabs:function(c){var b=this.editor;b.getSession().setUseSoftTabs(c);return this;},replaceSelection:function(j){var g=this.editor,h=g.getSession(),f,c,e,d=[],b=[];
